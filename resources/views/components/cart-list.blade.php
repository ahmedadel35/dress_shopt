@props([
    ...
    <li class="media border-bottom py-2 overflow-hidden" @if ($nocart) v-for="c in h.d.{{ $data }}" @else
        v-for="c in h.d.cart.{{ $data }}" @endif :key="'{{ random_int(2, 6555654) }}' + c.id">
        <img :src="c.product.images[0]" style="width: 6rem;"
            class="cartImage align-self-center mr-3 img-thumbnail bg-light tranistion" :alt="c.product.title">
            ...
            <div class="row">
                ...
                @if($hasRemove)
                    <span class="m-0 py-0 col-2 text-right">
                        <button type="button" class="close pl-2 pr-1 py-1 text-danger" aria-label="Close"
    ...
    @unless($nocart)
        <li v-for="cod in h.d.cart.loaders" :key="cod * Math.random()" class="border-bottom py-2 overflow-hidden">
            <content-loader width="{{ $shopList ? 450 : 350 }}"
                height="{{ $shopList ? 80 : 60 }}" primary-color="#cbcbcd" secondary-color="#02103b">
